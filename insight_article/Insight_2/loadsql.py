...
def parse_tables_from_query(query_text):
    tables_with_square_brackets_and_quotes = re.findall("(?:from|join)\s+(?:[0-9a-zA-Z\[\]\"\_\@\#\$]+\.)?([0-9a-zA-Z\[\]\"\_\@\#\$]+\.[0-9a-zA-Z\[\]\"\_\@\#\$]+)", query_text.lower())
    ...
    schema_filter = lambda pair: pair[0] not in ['sys','client','fds','fds_scratch','dbo', 'information_schema','guest']
...
def get_sql_q(filename,show =1,connection='SDF',skipSubsCheck=0):
    """
    ...
    if skipSubsCheck == 0:
        ...
        try:
            ...
            print("""Warning: Ensure you have an ODBC connection named SDF with access to the database\n\
        ...
        #check client's access to tables unless skipSubsCheck=1
        schema_query = """
    ...
    if skipSubsCheck == 1 or nulls.empty:
        if show == 1:
            print(sqlFile)
        return sqlFile
    else:
        print("""Please contact your FactSet representative for access to the package(s) used in this SQL query.\n\n\
